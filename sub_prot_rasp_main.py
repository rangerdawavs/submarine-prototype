...
while True:
    ...
    if(len(contours)>1):    #ckecks to make sure there are atleast two contours
		...
		stateVector[1]=calculus.der(xPostion_list,time_list)
		stateVector[2]=yPostion_list[-1]
		stateVector[3]=calculus.der(yPostion_list,time_list)
		stateVector[4]=Rotation_list[-1]
		stateVector[5]=calculus.der(Rotation_list,time_list)
